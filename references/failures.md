...
- **直し方**: ポジから `4koma` を外す。ネガに `4koma, 2koma, 3koma` を入れる。自然文では枚数の縦積みを書かず、配置とサイズだけ書く(full-width / vertical split / close-up / splash panel)。タグは `comic` のみ。語彙は [vocab.md](vocab.md) の構図節、書式は [anima-rules.md](anima-rules.md) の漫画ページ節
