...
## `torn pantyhose` が最初の裂け目より完成状態を強く出す (2026-09-08)

- **指示(日本語)**: ベッド上の女性が着用している黒いパンストを、男性が両手で左右へ引き、指の間に小さな裂け目が生まれた瞬間。スカートとパンストは残す
- **使ったプロンプト**: `black pantyhose, torn pantyhose, skirt lift, grabbing`。自然文で `His two hands pull one small section ... in opposite directions; a short fresh tear has just opened between his fingers`
- **外れた出力**: 2枚ともパンストを引っ張って破る主要な動作は読めたが、最初の小さな裂け目ではなく、複数または大きめの穴がすでに開いた状態になった。片方は一方の手が太腿をつかんだ
- **原因(推定)**: `torn pantyhose` は時間上の瞬間ではなく「すでに破れた」という結果状態を強く呼ぶ。破る前の着用状態、両手で引く途中、破った直後という三状態のうち、タグの完成状態が短い自然文より優先された。`grabbing` も対象を布に限定しない
- **直し方**: 破る意味だけが必要なら、破れを示す線と布をつまむ手が読める画像を採用し、穴の大きさは許容する。最初の小さな裂け目を厳密に見せる場合は、前ページを intact な布をつまむ状態、次ページを裂け目の端を片手でつまむ状態に分け、瞬間は書き文字 `ビリッ` で表す
## 画面外のネクタイへ手を伸ばすと女性の胸元に生える (2026-09-09)

- **指示(日本語)**: 女性だけを写し、画面外にいる男性のネクタイへ手を伸ばしかける
- **使ったプロンプト**: `upper body, reaching, looking at viewer`。自然文で `reaching toward the viewer's necktie`
- **外れた出力**: 3枚とも青いネクタイが女性の胸元中央から手前へ伸び、身体に付いた衣装のようになった。手もネクタイをつかまない
- **原因(推定)**: 画面外の所有者を描かず、小物だけを自然文で要求したため、最も近い女性の衣装へネクタイが結合した
- **直し方**: 小物が物語上必須でなければ捨て、`She begins reaching one open hand toward the camera`のように手の動作だけを書く。ネガに`necktie`。小物が必須なら男性の手や胸元も画面へ入れ、所有者を可視化する

## 画面外の男性の手首を女性が止めると、手の所有と動作が反転する (2026-09-09)

- **指示(日本語)**: 仰向けの女性が、画面下から入る男性の片腕を手首でつかみ、股間付近へ進む指先を止める
- **使ったプロンプト**: `pov, from above, wrist grab, hand on another's wrist`。自然文で男性は片腕だけ、女性が手首を包んで止めると指定。ネガに男性の胴・脚・ネクタイ
