...
タグと自然文の分担: **タグ=「何を描くか」(人数・キャラ・外見・服装)、自然文=「どこに・どう配置するか」(構図・空間・雰囲気)**。ハイブリッドが最良バランスという報告【未検証】(https://diffusiondoodles.substack.com/p/anima-light-fast-and-slightly-unruly)。自然文だと表記が揺れる語彙は [vocab.md](vocab.md) を参照してタグで書く。髪・瞳・ヘイローなどのアイデンティティはタグで一度だけ書き、自然文の先頭にも末尾にも繰り返さない。
...
- **容姿は一度だけ。** タグ行に髪・瞳・ヘイローなどが既にあるときは、自然文で外見を繰り返さない。`Rio has thigh-length...` を自然文の先頭にも末尾にも書かない。見本は [character-pose-change.md](../examples/_templates/character-pose-change.md)。
- **公式の「キャラ名を出したら続けて外見を描写する」は、タグに容姿がまだないときだけ使う**【公式をこのプロジェクトで限定】。見本は [two-girls-back-to-back.md](../examples/_templates/two-girls-back-to-back.md)。
...
詳細と見本は [manga-page.md](manga-page.md)。通常の3層ではなく、画風行 → Character → Panel文章にする。容姿は Character で一度だけ書き、各 Panel では髪・瞳・ヘイローを繰り返さない。コマ位置はタグ列挙ではなく文章で指定する。`4koma` は均等な縦積みになるので、不揃いのページでは使わない。
