...
3. **短い自然文(必要な場合だけ)** — 服の重なり、人物間の位置関係、カメラ、画面のどこまで写すかなど、タグだけでは曖昧な内容。容姿は書かない。

自然文が不要なら固定ブロック＋タグ行だけでよい。**タグで確定した内容を自然文で言い直さない。** たとえば `black hair, low twintails, blue eyes` と書いた後に、同じ髪と目を文章で再説明しない。髪・瞳・ヘイローなどのアイデンティティはタグで一度だけ書き、自然文の先頭にも末尾にも繰り返さない。公式はタグのランダムドロップアウト学習を明記しており、画像に関係するタグをすべて列挙する必要はない【公式】。

タグと自然文の分担は、**タグ=「何を描くか」(人数・キャラ・外見・服装)、自然文=「タグでは表しにくい関係や状態」(構図・空間・カメラ・雰囲気)**。ハイブリッドが最良バランスという報告【未検証】(https://diffusiondoodles.substack.com/p/anima-light-fast-and-slightly-unruly)。自然文だと表記が揺れる語彙は [vocab.md](vocab.md) を参照してタグにする。
...
- **容姿は一度だけ。** タグ行に髪・瞳・ヘイローなどが既にあるときは、自然文で外見・衣装・表情・構図を繰り返さない。`Rio has thigh-length...` を自然文の先頭にも末尾にも書かない。見本は [character-pose-change.md](../examples/_templates/character-pose-change.md)。
- 出したくない概念をポジティブ自然文へ書かない。`not gigantic`、`not elderly` のような否定は対象語を呼び込む可能性があるため、必要なら短いネガティブタグへ移す。
- **公式の「キャラ名を出したら続けて外見を描写する」は、タグに容姿がまだないときだけ使う**【公式をこのプロジェクトで限定】。複数キャラで名前だけだと特徴が混ざる。見本は [two-girls-back-to-back.md](../examples/_templates/two-girls-back-to-back.md)。
- 複数キャラの位置関係(left/right/behind等)は自然文で明示する。混ざり防止の外見差は自然文ではなく**タグで先に書く**【未検証】。
...
- **指示にない背景タグを足さない**。`simple background, gradient background, blurry background` は指示に無ければ入れない。室内描写と同居すると背景がブレンドして額や壁が消える【実生成・2026-09-07】。背景色は場面に合わせ、固定パレットにしない。
...
- **2人は外見を混ぜない。** 片方に長髪・ハロがあるなら、もう片方は短い別色の髪を**タグで先に書く**。自然文で容姿を再掲しない【実生成・2026-08-21】。
...
詳細と見本は [manga-page.md](manga-page.md)。通常の簡潔なハイブリッド形式ではなく、画風行 → Character → Panel文章にする。容姿は Character で一度だけ書き、各 Panel では髪・瞳・ヘイローを繰り返さない。コマ位置はタグ列挙ではなく文章で指定する。`4koma` は均等な縦積みになるので、不揃いのページでは使わない。
