...
Anima に漫画ページ専用 LoRA はない。コマ割りは **タグで種類、自然文で配置** に分ける。

- **均等な4コマ**(同じ大きさ・上から下): タグ `4koma`。これが欲しくないならポジに書かない。
- **不揃いの漫画ページ**(ワイド、縦割り、クローズアップ、大ゴマが混ざる): タグは `comic` のみ。`4koma` / `2koma` / `3koma` はネガへ。自然文で各コマの**位置と大きさ**を書く。`four stacked panels` のような枚数の縦積み表現は使わない(2026-08-21 実生成で縦積みになった)。
- フルカラーなら `colored`。`shoujo manga style` 等の manga style タグはモノクロ変換が強いので使わない。
- 吹き出し文字は1〜2単語まで。長い台詞は後入れ。
- コマが潰れる・混ざる場合は、1コマ1枚で生成して後で組む方が安定する。

## 日本語指示 → プロンプト変換の手順

1. **主題を確定**: 人数(`1girl`/`2girls`/`1boy`/`1other`)、キャラ名・作品名(既存キャラなら小文字タグで)
2. **外見・服装・表情・ポーズ**を Danbooru タグに変換([vocab.md](vocab.md) を参照)
3. **背景・構図・ライティング**は自然文で補足(空間関係はタグより自然文が得意)。漫画ページならコマの位置とサイズも自然文(「漫画ページ」節)
